...
<div class="container">
  ...
  <form class="form-horizontal" method="post" action="" id="form1">
   ...
   <!-- table showing filtered projects data also saves filters to variable for link to export -->
   <div style="width:65%">
     <table class="table table-striped" table style="margin-left:30px">
       <?php
         if (isset($_POST['Search'])) {
           ...
           while ($row = $result->fetch_array()) {
             print '<tr>';
             print '<td>' . $row["PROJECT_NUMBER"] . '</td><td>' . $row["PROJECT_NAME"] . '</td><td>' . $row["SPONSOR"] . '</td><td>' . $row["YEAR"] . '</td><td><a href="project-detail.php?id=' . $row['PROJECT_ID'] . '">View</a></td>';
             print '</tr>';
           }
         }
        ?>
